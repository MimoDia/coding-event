...
abstract class ProviderPluginBase extends PluginBase implements ProviderPluginInterface, ContainerFactoryPluginInterface {
   ...
   * @var string
   */
  protected $thumbsDirectory = 'public://audio_thumbnails';
  ...
  public function __construct($configuration, $plugin_id, $plugin_definition, ClientInterface $http_client) {
    ...
    if (!static::isApplicable($configuration['input'])) {
      throw new \Exception('Tried to create a audio provider plugin with invalid input.');
    }
    $this->input = $configuration['input'];
    $this->audioId = $this->getIdFromInput($configuration['input']);
  ...
  public function getLocalThumbnailUri() {
    return $this->thumbsDirectory . '/' . $this->getAudioId() . '.jpg';
  }
  ...
  public function getName() {
    return $this->t('@provider Audio (@id)', ['@provider' => $this->getPluginDefinition()['title'], '@id' => $this->getAudioId()]);
  }
